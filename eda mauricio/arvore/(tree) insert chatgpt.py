...
class Tree:
    ...
    def in_order_limit(self, node, limit):
        if node is not None:
            self.in_order_limit(node.esq, limit)
            self.fila.insert(node)
            self.in_order_limit(node.dir, limit)
